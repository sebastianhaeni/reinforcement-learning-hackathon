...
PATH_VIDEO   = './V4/video'
PATH_WEIGHTS = './V4/weigths'

FILE_EPSILON = Path('./V4/epsilon.log')
FILE_REWARDS = Path('./V4/rewards.log')
...
class Agent ():
    ...
    def learn (self):
        if len(self.memory) > self.batch_size:
            ...
            online_network = self.model

            action_state_value = online_network.predict_on_batch(states)

            next_action_state_value = rewards + self.gamma * np.amax(online_network.predict_on_batch(next_states),axis=1)*(1-dones)
            ...
            target_network = self.model

            action_state_value = target_network.predict_on_batch(states)

            target_network.fit(states, action_state_value, epochs=1, verbose=0)

            next_action_state_value = rewards + self.gamma * np.amax(target_network.predict_on_batch(next_states),axis=1)*(1-dones)
